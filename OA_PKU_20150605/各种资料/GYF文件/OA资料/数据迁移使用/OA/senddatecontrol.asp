...
if request.form("detel")="删除" and request.form("delid")<>"" then
	...
	wend
	'删除数据库中的记录
	sql = "update [senddate] set delforsend='no' where " & condition
	conn.Execute sql
	sql = "update [senddate] set delforsend='no'  where " & condition1
	conn.Execute sql
	sql = "update [senddate] set delforsend='no'  where " & condition2
	conn.Execute sql
end if
%>
<html>
...
set rs=Server.CreateObject("ADODB.recordset")
sql="select * from senddate,texttype where delforsend='yes' and senddate.documenttype=texttype.number order by id desc"
if request("lookfor")="yes" then sql="select * from senddate,texttype where delforsend='yes' and  senddate.documenttype=texttype.number and inputdate between " & "#" & mydate & "# and #" & mydate1 & "# order by id desc"
rs.open sql,conn,1
