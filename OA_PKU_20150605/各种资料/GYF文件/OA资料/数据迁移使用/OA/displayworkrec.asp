...
year1=year(recdate)
month1=month(recdate)
day1=day(recdate)
weekday1=weekday(recdate)
year2=year(now)
month2=month(now)
day2=day(now)
weekday2=weekday(now)
t=DateDiff("h",recdate, Now)
td=DateDiff("d",recdate, Now)
i=weekday(recdate)
'response.Write(t)
'response.Write("<br>")
'response.Write(weekday2)
'response.Write("<br>")
'response.Write(weekday1)
%>
						...
						<%
						if td<8 then
						  if (t<((7-i)*24+20) and day2<4) or oabusyname="赵师军" then

						%>
						                   <input type="submit" name="addworkrep" value="增加">
						<%
...
%>
                      ...
                      <!-- <%
						if td<8 then
						  if t<((7-i)*24+20) and day2<4 then
