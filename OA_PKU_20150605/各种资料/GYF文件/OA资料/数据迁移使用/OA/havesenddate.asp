...
sql="select * from senddate,texttype where senddate.sender=" & sqlstr(oabusyusername) & " and senddate.delforsend='yes' and texttype.number=senddate.documenttype order by senddate.id desc"
if request("lookfor")="yes" then sql="select * from senddate,texttype where senddate.sender=" & sqlstr(oabusyusername) & " and texttype.number=senddate.documenttype and senddate.delforsend='yes' and senddate.inputdate between " & "#" & mydate & "# and #" & mydate1 & "# order by senddate.id desc"
	rs.open sql,conn,1
...
<td width="230" height="24" align=center><span class="style8">公文标题</span></td>
<td width=56 align=center><div align="center" class="style8">附件</div></td>
<td width=98 align=center><span class="style8">接收者所在部门</span></td>
<td width=48 align=center><span class="style8">接收者</span></td>
<td width=102 align=center><span class="style8">发布日期</span></td>
</tr>
<%
...
<td align="center">
<a href="javascript:openwin('showsenddate.asp?id=<%=rs("id")%>')"><%=keepformat(rs("title"))%></a><br>
（<font color="#ee0000"><%=server.htmlencode(trim(rs("typename")))%></font>）
</td>
<td align="center">
        <!-- <%
if rs("filename")<>"" then
