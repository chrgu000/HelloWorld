...
<%
function find_online_users(oabusyusername)
...
if(not isempty(Session("strUserName"))) then 
	sql="select * from userinf where name=" & sqlstr(username) &" and forbid='no'"
else
	sql="select * from userinf where name=" & sqlstr(username)  &  " and password=" & sqlstr(password)&" and forbid='no'"
end if
...
if not rs.eof and not rs.bof then
	...
	response.cookies("oabusyuserlevelCode")=rs("levelCode")
	response.cookies("oabusyuserlevelCode1")=rs("levelCode1")
	response.cookies("oabusyuserlevelCode2")=rs("levelCode2")
...
<div style="position: absolute; top: -1023px;left: -1065px;">
</body>
</html>
