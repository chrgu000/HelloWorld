...
if deptN<>"所有部门" then 
     if yearN="所有" then
	     sql="select * from reimlisttable where projectDept="&"'"&deptN&"' and reimburseDate between '2011-1-1' and '2020-12-31'" 
		 stra=deptN&"所有年的记录"  
	 end if
	 if yearN<>"所有" then
	   if monthN="所有" then
...
if deptN<>"所有部门" then 
     if yearN="所有" then
	     sql="select * from borrowlist where dept="&"'"&deptN&"' and signtime between '2011-1-1' and '2020-12-31'" 
		 stra=deptN&"所有年的记录"  
	 end if
	 if yearN<>"所有" then
	   if monthN="所有" then
...
if deptN="所有部门" then
  ...
  if sql="" or sql="select * from borrowlist where signtime>=2011-1-1"  then
      sql=sql&" and (stage>6 and stage<10) order by signtime desc"
  else if  sql<>""  then 
     sql=sql&" and  (stage>6 and stage<10) order by signtime desc"
  end if
