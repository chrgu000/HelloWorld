...
currentpage=1
	...
	if yearN<>"所有" and monthN="所有" then
		...
		sql="select * from proinM where  time between '"&t1&"' and '"&t2&"'" 
		str=t1&"----"&t2&"之间的到款分配表"
	end if
...
'显示该页信息
 if currentpage=1 then
    showpage totalput,MaxPerPage,"manageinpay.asp"
	showContent
	showpage totalput,MaxPerPage,"manageinpay.asp"
 else 
    if(currentpage-1)*MaxPerPage<totalPut then
...
<th colspan="5" ><font color="#0d79b3" size="-1">查看项目预算决算表格</font></th>
<tr>
  <td><font color="#0d79b3" size="-1">编号</font></td>
  <td><font color="#0d79b3" size="-1">项目编号</font></td>
  <td><font color="#0d79b3" size="-1">增加时间</font></td>
  <td><font color="#0d79b3" size="-1">到款金额</font></td>
  <td><font color="#0d79b3" size="-1">分配额</font></td>
  <td><font color="#0d79b3" size="-1">付款单位</font></td>
...
<%
  ...
  <tr>
	...
	<td><A href=seeinpay.asp?id=<%=server.URLEncode(rs("id"))%>><%=rs("id")%></A></td>
	<td>&nbsp;<%=rs("projectId")%></td>
	<td>&nbsp;<%=rs("time")%></td>
	<td>&nbsp;<%=rs("money")%></td>
	<td>&nbsp;<%=rs("feipeiM")%></td>
	<td>&nbsp;<%=rs("inpaydanwei")%></td>
	<td>&nbsp;<%=rs("content")%></td>
	<td>&nbsp;<%=rs("pzh")%></td>
  </tr>
  <%
