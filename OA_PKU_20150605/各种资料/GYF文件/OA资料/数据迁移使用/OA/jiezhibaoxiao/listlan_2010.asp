...
if deptN="所有部门" or deptN="" then 
     if yearN="所有" then	 
	     sql="select * from reimlisttable where signtime between '2010-1-1' and '2010-12-31'" 
		 stra="所有的收支记录"	     
	 end if
	 if yearN<>"所有"  then
	   if monthN="" or monthN="所有" then
		...
		t2=DateSerial(yearN+1,1,1)
	     sql="select * from reimlisttable where  ((signtime between '"&t1&"' and '"&t2&"') or (gManagerApprovalTime  between '"&t1&"' and '"&t2&"'))" 
...
if deptN<>"所有部门" then 
     if yearN="所有" then
	     sql="select * from reimlisttable where projectDept="&"'"&deptN&"' and reimburseDate between '2010-1-1' and '2010-12-31'" 
		 stra=deptN&"所有年的记录"  
	 end if
	 if yearN<>"所有" then
	   if monthN="所有" then
	    t1=DateSerial(yearN,1,1)
		t2=DateSerial(yearN+1,1,1)
	     sql="select * from reimlisttable where projectDept="&"'"&deptN&"' and ((signtime between '"&t1&"' and '"&t2&"') or (gManagerApprovalTime  between '"&t1&"' and '"&t2&"'))" 
		 stra=deptN&yearN&"年的记录"  
	   end if
	   if monthN<>"所有" then
	    t1=DateSerial(yearN,monthN,1)
...
if deptN="所有部门" then
  ...
  if sql="" or sql="select * from reimlisttable where signtime between '2010-1-1' and '2010-12-31'"  then
