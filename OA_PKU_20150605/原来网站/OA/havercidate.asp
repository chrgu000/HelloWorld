...
sql="select * from senddate,texttype where (senddate.recipientusername=" & sqlstr(oabusyusername) & " or (senddate.recipientusername='所有人' and senddate.recipientuserdept=" & sqlstr(oabusyuserdept) & ")) and senddate.sender<>" & sqlstr(oabusyusername) & " and senddate.delforsend='yes' and senddate.documenttype=texttype.number order by id desc"
if request("lookfor")="yes" then sql="select * from senddate,texttype where (senddate.recipientusername=" & sqlstr(oabusyusername) & " or (senddate.recipientusername='所有人' and senddate.recipientuserdept=" & sqlstr(oabusyuserdept) & ")) and senddate.sender<>" & sqlstr(oabusyusername) & " and senddate.delfoesend='yes' and senddate.documenttype=texttype.number and inputdate between " & "#" & mydate & "# and #" & mydate1 & "# order by id desc"
	rs.open sql,conn,1
...
<br>
（<font color="#ee0000"><%=rs("typename")%></font>）</td>
<td align="center">
        <!--  <%
if rs("filename")<>"" then
%>
...
%>-->
        <!--#include file="showfile.asp"-->      </td><td align="center">
<%
