...
if request.form("detel")="删除" and request.form("delid")<>"" then
	   ...
	   'response.Write("<br>**"&idno&"**<br>")
	   set fso = server.CreateObject("Scripting.FileSystemObject")
       set rsd=server.createobject("adodb.recordset")
       sql="select * from senddate where id="&idno
       rsd.open sql,conn,1,2
	   'response.Write(rsd("filename"))
	   set fpath=Server.MapPath("./")      
	   'response.Write(Server.MapPath("./")&"\upload")
       if not rsd.eof then
	   'response.Write("&&"&trim(rsd("filename")))
	   ofile=fso.GetFile(Server.MapPath("./")&"\upload"&"\"&trim(rsd("filename")))
	   'response.Write("<br>我是ofile:*&&*"&ofile)
	   'response.Write("&&"&rsd("filename"))
       'fso.DeleteFile(patha&"\"&delfile)
...
if request.form("detel")="删除" and request.form("delid")<>"" then
	...
	conn.Execute sql
	sql = "delete * from senddate   where " & condition2
	conn.Execute sql
...
<center>
  ...
  <table>
<tr>
<form method=post action="senddatedel.asp">
...
if request("lookfor")="yes" then sql="select * from senddate,texttype where reid=0  and inputdate between " & "#" & mydate & "# and #" & mydate1 & "# order by id desc"
rs.open sql,conn,1
...
<%
	...
	sql="select name from userinf where  and username=" & sqlstr(rs("sender"))
	rs2.open sql,conn,1
