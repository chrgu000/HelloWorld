...
kk=request("deptarrMoney")
   ...
   sum=0
   for i=0 to n
   sum=sum+Cint(deptM(i))
   next
   response.Write(sum&"<br>"&money)
   if sum<>Cint(money) then
    response.write("<script language=""javascript"">")
...
if chittySum<>0 and money <>0 and sum=Cint(money) then
rs.addnew
...
if chittySum<>0 and money <>0 and tt<>"" then
rs1.addnew
for m=0 to 5
  rs1("Itemid")=tt
