...
if Trim(deptN)<>"所有部门" and deptN<>"" then
  wheresub=wheresub & "and projectdept='" & deptN & "'"
  stra=stra& deptN
end if 
  if yearN<>"所有"  then
       stra=stra&yearN&"年" 
...
sql="select * from reimlisttable_old where (stage>6 and stage<10)"
sql=sql & wheresub
 %>
<%=stra%><%'=t1%><%'=t2%><br /><%'=sql%>
...
<tr>
<td> &nbsp;<%=rs("id")%></td>
<td> &nbsp;<%if oabusyuserlevelCode="CN" OR oabusyuserlevelCode="CW" then %>
