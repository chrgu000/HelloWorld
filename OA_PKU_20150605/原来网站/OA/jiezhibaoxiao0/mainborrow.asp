...
set rsborr=server.CreateObject("adodb.recordset")
 if oabusyuserdept<>"综合管理部" then
     sqlborr="select * from borrowlist_old where dept = "& "'" & oabusyuserdept & "'" & "and stage =1"
 end if
  if oabusyuserdept="综合管理部" then
     sqlborr="select * from borrowlist_old where dept = "& "'" & oabusyuserdept & "'" & "and stage =1"
 end if
...
set cnborr=opendb("finance","conn","sql")
set rsborr=server.CreateObject("adodb.recordset")
sqlborr="select * from borrowlist_old where stage =3 "
rsborr.open sqlborr,cnborr,1,1
j=0
do while not rsborr.eof %>
...
sqlborr="select * from borrowlist_old where stage =4"
end if
if uStatus="DSZ" AND oabusyuserlevelCode1="BMJL" then
    if oabusyuserdept<>"综合管理部" then
     sqlborr="select * from borrowlist_old where stage =4 or (dept = "& "'" & oabusyuserdept & "'" & "and stage =1)"
    end if
    if oabusyuserdept="综合管理部" then
     sqlborr="select * from borrowlist_old where stage =4  or (dept = "& "'" & oabusyuserdept & "'" & "and stage =1)"
    end if
...
if uStatus="CN" then
sqlborr="select * from borrowlist_old where stage =7"
end if
rsborr.open sqlborr,cnborr,1,1
do while not rsborr.eof %>
...
set cnborr=opendb("finance","conn","sql")
set rsborr=server.CreateObject("adodb.recordset")
sqlborr="select * from borrowlist_old where borrower = "& "'" & oabusyusername & "'" & "and (stage =0 or stage=10)"
rsborr.open sqlborr,cnborr,1,1
i=0
do while not rsborr.eof %>
