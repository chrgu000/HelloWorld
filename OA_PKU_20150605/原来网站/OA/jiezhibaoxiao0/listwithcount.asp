...
financeCount=request("Count")
response.write("<div align=center>"&financeCount&"的记录</div>")
...
<form  name="form" action="listwithcount.asp" method="post">
日期
...
月
<input type="hidden" name="Count" value="<%=financeCount%>" />
...
<%
        ...
        set rs=server.createobject("adodb.recordset")
		   if (yearN="" and monthN="") or yearN="所有" then
		      sql="select *  from reimlisttable_old where givecount='"&financeCount&"'  and (stage>6 and stage<10)"
			...
			if yearN<>"所有" and yearN<>"" and monthN="所有" then
			   ...
			   sql="select *  from reimlisttable_old where givecount='"&financeCount&"' and reimburseDate>=#"&t1&"# and reimburseDate<=#"&t2&"#  and (stage>6 and stage<10)"
			   response.Write("<div align=center>"&yearN&"年该帐号所有记录</div>")
			end if
			if yearN<>"所有" and yearN<>"" and monthN<>"所有" then
			   t1=DateSerial(yearN,monthN,1)
		       t2=DateSerial(yearN,monthN+1,1)
			   sql="select *  from reimlisttable_old where givecount='"&financeCount&"' and (reimburseDate>=#"&t1&"# and reimburseDate<=#"&t2&"#)  and (stage>6 and stage<10)"
			   response.Write("<div align=center>"&yearN&"年"&monthN&"月该帐号所有记录</div>")
			end if
		'response.Write(t1)
'		response.Write(t2)
