...
<option vlue="其他"      <%=selected("其他",dept)%>>其他</option>
</select>
日期
...
'getsql dept1,year1,month1 
  if deptN="所有部门" then 
     if yearN="所有" then
	   sql1="select * from borrowlist_old where stage>1 and stage<6 and finance="&"'"&oabusyusername&"'"
	 ...
	 if yearN<>"所有" then
	   if monthN="" then
	    ...
	    sql1="select * from borrowlist_old where stage>1 and stage<6 and date>=#"&t1&"# and date<#"&t2&"# and finance="&"'"& oabusyusername&"'"
  ...
  if deptN<>"所有部门"  and deptN<>"" then 
     if yearN="所有" then
	   sql1="select * from borrowlist_old where stage>1 and stage<6 and dept="&"'"&deptN&"' and finance="&"'"&oabusyusername&"'"
	   response.Write(deptN&yearN&"年的申请")
	 end if
	 if yearN<>"所有" and yearN<>"" then
	   if monthN="" then
	    t1=DateSerial(yearN,1,1)
		t2=DateSerial(yearN+1,1,1)
	    sql1="select * from borrowlist_old where stage>1 and stage<6 and date>=#"&t1&"# and date<#"&t2&"# and dept="&"'"&deptN&"' and finance="&"'"&oabusyusername&"'"
		response.Write(deptN&yearN&"年的申请")
	   end if
	   if monthN<>"" then
	    t1=DateSerial(yearN,monthN,1)
...
'显示该页信息
 if currentpage=1 then
    showpage totalput,MaxPerPage,"seeManageReimList2.asp"
	showContent
	showpage totalput,MaxPerPage,"seeManageReimList2.asp"
 else 
    if(currentpage-1)*MaxPerPage<totalPut then
...
function getsql(deptN,yearN,monthN)
  if deptN="所有部门" then 
     if yearN="所有" then
	   response.Write("select * from borrowlist_old where stage>1 and stage<6")
	 end if
	 if yearN<>"所有" then
	   if monthN="" then
	    t1=DateSerial(yearN,1,1)
		t2=DateSerial(yearN+1,1,1)
	    response.Write("select * from borrowlist_old where stage>1 and stage<6 and date>"&t1&"and date<"&t2)
	   end if
	   if monthN<>"" then
	    t1=DateSerial(yearN,monthN,1)
		t2=DateSerial(yearN,monthN+1,1)
  ...
  if deptN<>"所有部门" then 
     if yearN="所有" then
	   response.Write("select * from borrowlist_old where stage>1 and stage<6 and dept="&deptN)
	 end if
	 if yearN<>"所有" then
	   if monthN="" then
	    t1=DateSerial(yearN,1,1)
		t2=DateSerial(yearN+1,1,1)
	    response.Write("select * from borrowlist_old where stage>1 and stage<6 and date>"&t1&"and date<"&t2&"and dept="&deptN)
	   end if
	   if monthN<>"" then
	    t1=DateSerial(yearN,monthN,1)
		t2=DateSerial(yearN,monthN+1,1)
