...
if oabusyuserdept<>"综合管理部" then
 '首先是非项目成本自己填写的、别人填写的和被退回的已完成；然后是项目成本 自己是项目经理的但不是项目负责人已完成，项目成本的项目负责人是自己，提交的或未提交的已完成
  sqlborr="select * from reimlisttable_old where (type='A' and projectDept = "& "'" & oabusyuserdept & "' and (stage=1  or stage=2)) or (type='B'and ((projectDept = "& "'" & oabusyuserdept & "' and  stage =2 ) or(dept = "& "'" & oabusyuserdept & "' and (stage=1 or (claimant="&"'"&oabusyusername&"' and stage=0) ))))"
 end if
if oabusyuserdept="综合管理部" then
 sqlborr="select * from reimlisttable_old where stage=13 or (type='A' and projectDept = "& "'" & oabusyuserdept & "'" & "and (stage =1  or stage=2 )) or (type='B' and projectDept = "& "'" & oabusyuserdept & "'" & "and stage =2 ) or (type='B' and principal = "& "'" & oabusyusername & "'" & "and (stage =1 or stage =0) )"
...
'changed by zhaoshijun with qq:515447782   @ 2010-2-24 for update database
set rsborr=server.CreateObject("adodb.recordset")
sqlborr="select * from reimlisttable_old where stage=3 or (stage=6 and ch<>'') or (type='A' and dept = "& "'" & oabusyuserdept & "' and stage =1 or (claimant="&"'"&oabusyusername&"' and stage=0 ))"
rsborr.open sqlborr,cnborr,1,1
j=0
do while not rsborr.eof %>
...
sqlborr="select * from reimlisttable_old where stage =7 or (type='A' and dept = "& "'" & oabusyuserdept & "'" & "and ((claimant="&"'"&oabusyusername&"' and stage=0)))"
'sqlborr="select * from reimlisttable_old where  (type='A' and dept = "& "'" & oabusyuserdept & "'" & "and ((claimant="&"'"&oabusyusername&"' and stage=0)))"
rsborr.open sqlborr,cnborr,1,1
j=0
do while not rsborr.eof %>
...
sqlborr="select * from reimlisttable_old where stage=4 "
end if
if uStatus="DSZ" and oabusyuserlevelCode1="BMJL" then
sqlborr="select * from reimlisttable_old where (stage =4 and relclaimant<>'陈秀万' ) or (type='A' and (projectDept = '产品部' or projectDept = '工程部') and (stage=1  or stage=2)) or (type='B'and (((projectDept = '产品部' or projectDept = '工程部') and  stage =2 ) or((projectDept = '产品部' or projectDept = '工程部') and (stage=1 or (claimant="&"'"&oabusyusername&"' and stage=0))))) "
'2010 年2月1日 将陈老师的功能修改回来 zhaoshijun
'sqlborr="select * from reimlisttable_old where (stage =4  ) or (type='A' and (projectDept = '产品部' or projectDept = '工程部') and (stage=1  or stage=2)) or (type='B'and (((projectDept = '产品部' or projectDept = '工程部') and  stage =2 ) or((projectDept = '产品部' or projectDept = '工程部') and (stage=1 or (claimant="&"'"&oabusyusername&"' and stage=0))))) "
end if
if uStatus="DSZ" and oabusyusername="陈秀万"  then
sqlborr="select * from reimlisttable_old where stage =4 and relclaimant='陈秀万'"
end if
if uStatus="CW" then
'sqlborr="select * from reimlisttable_old where (stage=8 or stage=9 or stage=5) and ch='N'" '这个是2010-12-24之前使用的语句
sqlborr="select * from reimlisttable_old where stage=5 and ch='N'" '这个是2010-12-24之后使用的语句
...
sqlborr="select * from reimlisttable_old  where (type='A'and  claimant = "& "'" & oabusyusername & "' and (stage = 10 or stage = 7)) or (type='B'and claimant = "& "'" & oabusyusername & "' and (stage = 10 or stage = 7))  or (claimant="&"'"&oabusyusername&"' and stage=0) or (stage=1 and principal= "& "'" & oabusyusername & "')"
rsborr.open sqlborr,cnborr,1,1
i=0
do while not rsborr.eof %>
