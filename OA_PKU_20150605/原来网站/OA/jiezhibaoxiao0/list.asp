	 ...
	 if yearN<>"所有"  then
	   if monthN="" or monthN="所有" then
		...
		t2=DateSerial(yearN+1,1,1)
	     sql="select * from reimlisttable_old where  occurDate>=#"&t1&"# and occurDate<#"&t2&"#  "
...
if deptN<>"所有部门" then 
     if yearN="所有" then
	     sql="select * from reimlisttable_old where projectDept="&"'"&deptN&"'" 
		 stra=deptN&"所有年的记录"  
	 end if
	 if yearN<>"所有" then
	   if monthN="所有" then
	    t1=DateSerial(yearN,1,1)
		t2=DateSerial(yearN+1,1,1)
	     sql="select * from reimlisttable_old where projectDept="&"'"&deptN&"' and occurDate>=#"&t1&"# and occurDate<#"&t2&"#" 
		 stra=deptN&yearN&"年的记录"  
	   end if
	   if monthN<>"所有" then
	    t1=DateSerial(yearN,monthN,1)
...
if deptN="所有部门" then
  ...
  if sql="" or sql="select * from reimlisttable_old" then
...
<%      set conn=opendb("GFProjectType","conn","accessdsn")
        set rs=server.createobject("adodb.recordset")
        sql="select financeCount from projectType_old where projectId='"&projectId(ff)&"'"
        rs.open sql,conn,1,1
        do while not rs.eof and not rs.bof
%>
...
<%      set conn=opendb("GFProjectType","conn","accessdsn")
        set rs=server.createobject("adodb.recordset")
        sql="select dept from projectType_old where projectId='"&projectId(ff)&"'"
        rs.open sql,conn,1,1
        do while not rs.eof and not rs.bof
