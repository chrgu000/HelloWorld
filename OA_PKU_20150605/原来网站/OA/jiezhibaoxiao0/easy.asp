...
<%
	...
	if saveflag="保存" then
		...
		sql="select * from borrowlist_old where id="&CInt(idflag)
...
'					 
'					sql="select * from borrowlist_old where stage=7 and borrowTime between '"&t1&"' and '"&t2&"'"
'				end if
'				if CInt(monthG)<>CInt(13) then
...
'			end if
'			if deptG<>"所有部门" then
'				'sql="select * from borrowlist_old where stage=7"
'				if CInt(monthG)=CInt(13) then
'					 t1=DateSerial(yearG,1,1)
'					 t2=DateSerial(yearG+1,1,1)
'					 
'					sql="select * from borrowlist_old where stage=7 and borrowTime between '"&t1&"' and '"&t2&"' and dept='"&deptG&"' order by borrower desc"
'				end if
'				if CInt(monthG)<>CInt(13) then
'					 t1=DateSerial(yearG,monthG,1)
'					 t2=DateSerial(yearG,monthG+1,1)
...
'		end if
'		if nameG<>"" then
'			sql="select * from borrowlist_old where stage=7 and borrower='"&nameG&"' order by borrowTime desc"
'		end if
'		if tG<>"" then
'			if tG="第一次还款" then
'				sql="select * from borrowlist_old where stage=7 and frefound=0 and srefound=0 and trefound=0"
'			end if
'			if tG="第二次还款" then
'				sql="select * from borrowlist_old where stage=7 and frefound<>0 and srefound=0 and trefound=0"
'			end if
...
'		end if
		'if flagG<>"" then
'			if flagG="未还清" then
				sql="select * from borrowlist_old where stage=15 order by borrowTime desc"				
			'end if
'			if flagG="已还清" then
'				sql="select * from borrowlist_old where stage=8 order by borrowTime desc"
'			end if
'		end if
		'sql="select * from borrowlist_old where stage=7 and borrowTime between '2010-3-1' and '2010-4-1'"
		'response.Write(sql)
	...
	<form action="easy.asp">
		...
		%>
		<input type="submit" name="save"   value="保存" /> <!--name="conf5"-->
		</td>
		<td>
		<% if rs("stage")=8 then%>
