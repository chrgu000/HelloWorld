...
<%
	if not rs1.eof and not rs1.bof then
		...
		username=oabusyname '2010年2月2日 修改 -----页面增加链表 填写月未考勤原因
	end if
...
<%
	...
	wend
%>
<option value="公司全部"<%=selected(userdept,"公司全部")%>>全体人员</option>
</select>
	<%
	'lisonglin 09-6-26 12:11 start
	dim ui
	dim users()
	dim shownames()
	dim depts()
	if(userdept="公司全部") then
	%>
		<center>
		...
		<table border="1" cellpadding="0" cellspacing="0" width="540" bordercolorlight="#B0C8EA" bordercolordark="#FFFFFF">
		  <tr bgcolor="D7E8F8">
			<td width="104" height="30" align="center">人员</td>
			<td width="66" height="30" align="center">上午未到</td>
			<td width="70" height="30" align="center">上午迟到</td>
			<td width="70" height="30" align="center">下午早退</td>
		 </tr>
		<%
		'读取全体人员名单
		set rs1=server.createobject("adodb.recordset")
		sql="select DISTINCT username,name,userdept from userinf"
		rs1.open sql,conn,1
		ui=0
		while not rs1.eof and not rs1.bof
			rs1.movenext
		...
		if(ui>0) then
			redim users(ui-1)
			redim shownames(ui-1)
			redim depts(ui-1)
			ui=0
			while not rs1.eof and not rs1.bof
				users(ui)=cstr(rs1("username"))
				shownames(ui)=cstr(rs1("name"))
				depts(ui)=cstr(rs1("userdept"))
				rs1.movenext
				ui=ui+1
			wend
			rs1.movefirst
		...
		for j=0 to UBound(users) 
			...
			%>
			<tr>
				<td width="66" height="25"><font color="#0000ee"><%=cstr(shownames(j))%></font></td>
	...
	<select size=1 name="username">
		...
		'09-6-26 11:37 lisonglin start
		if(username="部门全部") then
			ui=0
			while not rs1.eof and not rs1.bof
				rs1.movenext
				ui=ui+1
		...
		set rs1=nothing
		%>
		<option value="部门全部"<%=selected(username,"部门全部")%>>全部</option>
		</select>
		...
		%>
		<!-- zhaoshijun 2009-6-30 16:00 start-->
	...
	<input type="submit" value="查看">
	   ...
	   <input type="button" name="printbtn" value="打印" onClick="printsub();">
	   <%end if%>
	   <!-- zhaoshijun 2009-6-30 16:00 end-->
</td>
</form>
</tr>
</table>
...
'lisonglin 09-6-26 11:43 start
'if username<>"" then 
if username<>"" and username<>"部门全部" then
'lisonglin 09-6-26 11:43 end
%>
<center>
<font color="#dd0000" size="+1"><%=namevalue%></font><font size="+1"><%=cstr(yearvalue)&"年"&cstr(monthvalue)&"月考勤统计表"%></font></center>
<br>
...
<div align="center" id="divx" style="display:none"><!--<a href="writemonthreason.asp" target="_self">--><font color="#FF0000" >请点击填写本月未考勤及迟到原因</font><!--</a>--></div>
<table width="540" border="1" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#B0C8EA" bordercolordark="#FFFFFF">
  ...
  <tr>
    ...
    <td width="66" height="25" onMouseOver="over();" onMouseOut="over2();"><div align="center"><font color="#0000ee"><a href="writemonthreason.asp"  ><%=cstr(amnocomesums)%></font>次</a>&nbsp;</div></td>
