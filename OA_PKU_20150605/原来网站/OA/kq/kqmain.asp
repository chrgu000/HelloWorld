...
function bekq()
{
    confirm("温馨提示：请记得填写工作计划和工作总结或编辑完成情况");
...
function bekq1()
{
    confirm("o(∩_∩)o 请先停止数据库后，备份数据库，然后再下班！！请记得填写工作计划和工作总结或编辑完成情况");
	winhandle=window.open('bekq.asp','handkqwin','width=450,height=380,toolbar=no,scrollbars=no,resizable=0,menubar=no');
}
...
%>
<p align="center">
<% if oabusyname<>"赵师军" then%>
<input type="button" value="开始考勤" onClick="bekq()">
&nbsp;&nbsp;&nbsp;
<input name="button" type="button" onClick="handkq()" value="补 考 勤">
<%end if%>
<% if oabusyname="赵师军" then%>
<input type="button" value="开始考勤" onClick="bekq1()">
&nbsp;&nbsp;&nbsp;
<input name="button" type="button" onClick="handkq1()" value="补 考 勤">
<%end if%>
</p>
<%' if oabusyname="赵师军" and ((pmgotimephase1<=nowtime and pmgotimephase2>=nowtime) or (amcometimephase1<=nowtime and amcometimephase2>=nowtime)) then%>
<% if oabusyname="赵师军" then%>
<div align="center"><a href="../jiezhibaoxiao/backup2.asp"><font color="#000099" size="+1">备份文件</font></a>
<br><br>
<a href="../backall.asp">备份整个OA文件夹（请在每个周末的时候备份，，并且在周一的时候转移到别的机器（释放控件））</a>
</div>
<%end if%>
</body>
</html>
