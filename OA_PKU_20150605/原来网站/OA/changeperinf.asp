...
'下面的作用就是 先检查数据库中是不是包含该员工的信息，如果还没有则增加一条记录
...
set rs=server.createobject("adodb.recordset")
sql="select * from personinf where username="&"'"&username&"'"
...
<center>
  ...
  <table bordercolorlight="#C0C0C0" bordercolordark="#FFFFFF">
    <tr> 
      <td> 编辑<%=username%>的个人基本档案&nbsp;&nbsp; </td>
      <!--<td>  
	   <form method="post" action="stafpersoninf.asp">
        <input type="submit" value="返回">
		</form>
      </td> -->
        <td>  
		<form method="post" action="delpersoninf.asp">
          <input type="hidden" name="username" value="<%=username%>"><input type="submit" name="submit" value="删除" onClick="return window.confirm('你确定要删除这条基本档案吗？');">
        </form>
		</td>
    </tr>
  </table>
  ...
  <table border="0" cellpadding="0" cellspacing="0" width="540" bordercolorlight="#C0C0C0" bordercolordark="#FFFFFF">
    <tr> 
      <td align="center" style="border-left: 2 solid #B0C8EA; border-right: 1 solid #B0C8EA; border-top: 2 solid #B0C8EA; border-bottom: 1 solid #B0C8EA" width="15%" height="20">姓&nbsp;&nbsp;&nbsp; 
        名</td>
      <td style="border-right: 1 solid #B0C8EA; border-top: 2 solid #B0C8EA; border-bottom: 1 solid #B0C8EA" width="35%"><input type=text size=10 name=username value="<%=username%>"></td>
    ...
    <tr> 
	 ...
	 <%
		  ...
		  set conn=opendb("oabusy","conn","accessdsn")
			set rs=server.createobject("adodb.recordset")
	      sql="select * from userinf where username="&"'"&request("username")&"'"
		  rs.open sql,conn,1,1
		  if not rs.eof then
	  %>
	  <input type=text size=10 name=userdept value="<%=rs("userdept")%>">
	  </td>
      <td align="center" style="border-right: 1 solid #B0C8EA; border-bottom: 1 solid #B0C8EA">职&nbsp;&nbsp;&nbsp; 
        务</td>
      <td style="border-right: 2 solid #B0C8EA; border-bottom: 1 solid #B0C8EA">
	  <input type=text size=10 name=userlevel value="<%=rs("userlevel")%>">
	  <%end if%>
	  </td>
    </tr>
    <tr> 
