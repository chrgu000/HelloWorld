...
sl="select * from userinf where userdept ="&"'"& getdept(rs("borrowerdeptcode"))&"'"&" and (levelCode= 'BMJL' or levelCode= 'DSZ' or levelCode1= 'BMJL') and forbid='no'"
r.open sl,cn,1,1
%>
...
<tr>
<td>编号</td><td><%=rs("id")%></td><td>申请人</td><td><%=rs("borrower")%></td><td>部门</td><td><%=getdept(rs("borrowerdeptcode"))%></td><td>部门负责人</td>
<td>
...
<tr>
<td>申请日期</td><td colspan="5"><%=rs("asktime")%></td><td>金额（大写）</td><td><input type="text" name="Bmoney" ></td>
</tr>
<tr>
<td>事由</td><td colspan="7"><%=rs("content")&"&nbsp;&nbsp;"%></td>
</tr>
<tr>
<td>审批意见</td><td colspan="7">
<%
if rs("notes")<>"" then
