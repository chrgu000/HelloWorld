...
<div align="center">
<form  name="form" action="seeAllReimListforM.asp" method="post">
日期
...
set rs=server.CreateObject("adodb.recordset")
     ...
     if yearN="所有" then
	   sql1="select * from ReimMilti where Stage>0 "
	 ...
	 if yearN<>"所有" then
	   if monthN="" then
	    ...
	    sql1="select * from ReimMilti where Stage>0 and ReimDate>='"&t1&"' and ReimDate<'"&t2&"' "  'change by junnor @20131213 from # to "'"
		sql="select * from ReimMilti where  ReimDate>='"&t1&"' and ReimDate<'"&t2&"' "
		response.Write(deptN&yearN&"年的申请")
	   end if
	   if monthN<>"" then
	    t1=DateSerial(yearN,monthN,1)
		t2=DateSerial(yearN,monthN+1,1)
	    sql1="select * from ReimMilti where Stage>0 and ReimDate>='"&t1&"' and ReimDate<'"&t2&"' "
		sql="select * from ReimMilti where  ReimDate>='"&t1&"' and ReimDate<'"&t2&"' "
