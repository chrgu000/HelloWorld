...
dim bumen
dim recestr
for each eve in mysendto
'response.Write(eve)
if instr(eve,"所有人")>0 then 
   dept=split(eve,":")
   bumen=dept(0)
   set conn=opendb("oabusy","conn","accessdsn")
   set rs=server.createobject("ADODB.recordset") 
   sql="select * from userinf where userdept="&"'"&bumen&"'"
   rs.open sql,conn,1,3
   do while not rs.eof
   recestr=recestr&rs("username")&")"
   rs.movenext
   loop
else 
   if len(eve)>0 then
   sendtolen=Instr(eve,"(")
   allsendlen=len(eve)
   sentopep=right(eve,allsendlen-sendtolen)
   recestr=recestr&sentopep
   'response.Write("ddddd<br>")
   'response.Write(sendtolen&"<br>")
   'response.Write(allsendlen&"<br>")
   'response.Write(sentopep&"<br>")
   'response.Write(recestr&"<br>")
end if
end if
next
'response.Write(bumen)
'response.Write(recestr)
