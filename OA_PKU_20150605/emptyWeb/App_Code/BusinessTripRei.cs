...
/// 出差报销申请表
/// </summary>
public class BusinessTripRei
{
	public BusinessTripRei()
	{
		//
		// TODO: 在此处添加构造函数逻辑
		//
    ...
    public static bool HasBusinessTripRei(int id)
    {
        bool ret;
        SqlConnection conn = new SqlConnection(connString);
        SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM [BusinessTripRei] WHERE [RecordID] = @RecordID", conn);
        cmd.Parameters.Add("@RecordID", SqlDbType.Int).Value = id;
        conn.Open();
    ...
    private double moneyRei;
    public double MoneyRei { get { return this.moneyRei; } set { this.moneyRei = value; } }
